...
class WebsiteStockConfigSettings(models.Model):
    ...
    def toggle_is_active(self):
        for record in self:
            ...
            [('is_active', '=', True), ('id', 'not in', [record.id]),('website_id', '=', record.website_id.id)])
            if active_ids:
                raise UserError("Currently a Configuration Setting for Website is active.")
    ...
    def set_product_default_value(self):
        ...
        for rec in record:
            product = self.env['product.product'].search([('website_id','in',[rec.website_id.id,False])])
            if rec.wk_deny_order:
