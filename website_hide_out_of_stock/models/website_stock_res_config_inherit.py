...
class ProductTemplate(models.Model):
    ...
    def activate_cron_template(self):
        cron_obj = self.env['ir.model.data'].get_object_reference('website_hide_out_of_stock', 'ir_cron_scheduler_avalability')
        cron = self.env["ir.cron"].browse(int(cron_obj[1]))
    ...
    def open_cron(self):
        cron_id = self.env['ir.model.data'].get_object_reference('website_hide_out_of_stock', 'ir_cron_scheduler_avalability')
        return {
